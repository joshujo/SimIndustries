...
use crate::{core::{Assets, Company, Factory, Inventory, Player, WorldData}, interface::retrieve_data::RetrieveData, logic::game::systems::produce_goods};
...
fn emit_time(world_data: &mut WorldData, app: &AppHandle) {
    ...
    let (day, month, year, hour, minute, second) = {
        (
            format!("{:02}", date.day()),
            format!("{:02}",(date.month() as usize + 1)),
            format!("{:02}", date.year()),
            format!("{:02}", time.hour()),
...
fn emit_data(world_data: &mut WorldData, app: &AppHandle) {
    ...
    if state.data.contains(&crate::interface::retrieve_data::RetrieveType::PlayerAssets) {
        let (_, assets) = world_data.world.borrow::<(View<Player>, View<Assets>)>().unwrap();
        let player_assets = &assets.iter().next().unwrap().0;
        
        let assets: Vec<_> = player_assets.par_iter().map(|k| {
